...
subroutine compute_T_jk_P(T_jk,delr,delp)
  ...
  do i1=1,nquant
    do j1=1,nquant
      sigma(i1,j1)=ci(i1)*dconjg(ci(j1))
    enddo
  enddo
...
subroutine write_average
  ...
  do i=1,nsteps/nstep_avg
    write(100,'(16f15.7)')(i-1)*nstep_avg*dtc*1.d15,real(rho(1,1,i)),real(rho(1,2,i)),real(rho(2,2,i)),real(rho(2,3,i)),real(rho(3,3,i)),real(rho(1,3,i))
  enddo
...
subroutine setup_parameters
  ...
  omg=omg_B
  g_coup=dsqrt(2d0*lambda_B*mass(1)*omg_B**2)
  ...
  Hamil_site(3,3)=210.d0


  Hamil_site(1,2)=-87.7d0; Hamil_site(2,1)=Hamil_site(1,2)
  Hamil_site(2,3)=30.8d0; Hamil_site(3,2)=Hamil_site(2,3)
  Hamil_site(1,3)=5.5d0; Hamil_site(3,1)=Hamil_site(1,3)

  Hamil_site=Hamil_site*wave_to_J

  !do i=1,nclass
  !  omg(i)=i*omg_max/real(nclass)
  !  ck(i)=dsqrt(2*spectral(omg(i))*mass(i)*omg(i)*delw/pi)
!write(20,*) omg(i)/(2*pi*clight),spectral(omg(i))/wave_to_J
  !enddo
!write(6,*) 2*sum(ck**2/(mass*omg**2))/wave_to_J
...
! write(*,*) omg_B/(2*pi*clight),lambda_B/wave_to_j
! write(*,*) H_diab/wave_to_j
!stop 
